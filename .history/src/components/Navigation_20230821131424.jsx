...
const Navigation = () => {
  ...
  const userWalletSplice = userWallet ? `${userWallet.slice(0, 6)}...${userWallet.slice(-4)}` : null
  ...
  return (
    <nav>
      ...
      <button className='connect-wallet__button' onClick={handleConnectEthers}>{userWallet ? 'Connected' : 'Connect'}</button>
    </nav>
  )
}
