...
import { useCrazyFacesMinting } from '../context'

const NftMintModal = ({ handleNFTMint, nftMintModalIsOpen, userERC20Balance, onClose }) => {
    const { nftMintPrice } = useCrazyFacesMinting()
    console.log('NftMintModal - nftMintPrice: ', Math.floor(ethers.utils.formatEther(nftMintPrice?.toString())))
    // console.log('NftMintModal - nftMintPrice: ', ethers.utils.formatEther(nftMintPrice.toString()))
    const [quantityToMint, setQuantityToMint] = useState(0)
    
    if (!nftMintModalIsOpen) return null

    const maxNFTsCanMint = Math.floor(userERC20Balance / 100) 
    ...
    const handleMintNFTs = () => {
        ...
        handleNFTMint(quantityToMint)
        console.log('NftMintModal - called `handleNFTMint()`')
        setQuantityToMint(0)
    }
