    ...
    if (turtleCatCoinContract) {
      const mintERC20 = async () => {
        try {
          ...
          const transactionReceipt = transaction.wait
          if(transactionReceipt) {
            console.log('TokenManagement - transactionReceipt: ', transactionReceipt)
            console.log('TokenManagement (handleWBCMint()) - useEffectTrigger: ', useEffectTrigger)
            setUseEffectTrigger(prevState => !prevState)
            console.log('TokenManagement (handleWBCMint()) - setUseEffectTrigger has run...')
          }
        } catch (err) {
