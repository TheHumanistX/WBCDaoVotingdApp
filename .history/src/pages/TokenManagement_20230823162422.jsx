...
const TokenManagement = () => {
  const { CRAZYFACES_MINTING_CONTRACT_ADDRESS, nftMintPrice } = useCrazyFacesMinting();
  ...
  const handleNFTMint = async (quantityToMint) => {
    ...
    if (turtleCatCoinContract && crazyFacesContract) {
      try {
        ...
        const totalNFTMintPrice = await crazyFacesMintingContract.functions[signatureWithArgs](amount);
        const approval = await turtleCatCoinContract.approve(CRAZYFACES_MINTING_CONTRACT_ADDRESS, nftMintPrice)
