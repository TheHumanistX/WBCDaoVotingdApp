...
const TokenManagement = () => {
  ...
  const handleNFTMint = async (quantityToMint) => {
    ...
    if (turtleCatCoinContract && crazyFacesMintingContract) {
      try {
        ...
        console.log('TokenManagement - handleNFTMing - totalNFTMintPrice: ', ethers.utils.formatEther(totalNFTMintPrice[0]))
        const totalNFTMintPriceFormatted = ethers.utils.formatEther(totalNFTMintPrice)
        console.log('TokenManagement - handleNFTMing - totalNFTMintPriceFormatted: ', totalNFTMintPriceFormatted)
        const approvalResponse = await turtleCatCoinContract.approve(CRAZYFACES_MINTING_CONTRACT_ADDRESS, ethers.utils.formatEther(totalNFTMintPrice[0]))
